...
require_once(dirname(__FILE__) . '/expresspayepos_files/ExpressPayEripHelper.php');
require_once(dirname(__FILE__) . '/expresspayepos_files/ExpressPayEripLog.php');
...
if (defined('PAYMENT_NOTIFICATION')) {
    ...
    $success_url        = fn_url("payment_notification.success?payment=expresspayepos&order_id=" . $order_id, AREA, 'current');
    $fail_url           = fn_url("payment_notification.fail?payment=expresspayepos&order_id=" . $order_id, AREA, 'current');
    ...
    if (isset($response_erip['Errors'])) {
        ...
        $_SESSION['module'] = "epos";
        $_SESSION['status'] = "ok";
        $_SESSION['order_id'] = $payment_settings["serviceEpos"] .'-1-'. $order_id  ;
        ...
        $response = array();
        $response['order_status'] = 'O';
        $response['reason_text'] = "_Ждет оплаты в EPOS";
        $response['paid_amount'] = $amount;
        fn_change_order_status($order_id, 'O');
